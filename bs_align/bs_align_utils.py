...
def process_aligner_output(filename, pair_end = False):
    ...
    if format == BOWTIE or format == BOWTIE2 or format == SNAP:
        if pair_end:
            for line in input:
		    ...
		    #print line
		    #print '@@@@@@@@@@@@@@@@@@@'
		    #print header, chr, location, no_mismatch, cigar
                    yield header, chr, location, no_mismatch, cigar, line
    elif format == SOAP:
        if pair_end:
            for line in input:
