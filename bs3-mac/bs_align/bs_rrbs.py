...
def bs_rrbs(aligner, main_read_file, asktag, adapter_file, cut_s, cut_e, no_small_lines, max_mismatch_no,
    ...
    if asktag=="N" :
        ...
        for read_file in isplit_file(main_read_file, tmp_d(input_fname)+'-s-', no_small_lines):
            ...
            outfile2 = '/Users/kyypgh/Downloads/BSseeker2-master/tmp'
            outf2=open(outfile2 + 's','w')
            ...
            if (aligner == 'snap'):
                cmd = 'perl fasta_to_fastq.pl ' + outfile2 + 's' + ' > ' + outfile2
    ...
    # ====================================================
        ...
        for read_file in isplit_file(main_read_file, tmp_d(input_fname)+'-s-', no_small_lines):
            ...
            if (aligner == 'snap'):
                cmd = 'perl fasta_to_fastq.pl ' + outfile2 + 's' + ' > ' + outfile2
                subprocess.Popen(cmd, shell=True)
                cmd = 'perl fasta_to_fastq.pl ' + outfile3 + 's' + ' > ' + outfile3
                subprocess.Popen(cmd, shell=True)
                input_format = 'fastq'
